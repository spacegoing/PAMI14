...
if (options.K > 1),
    switch options.learningQP,
        case 1
            ...
            A = [A; zeros(size(Dxx, 1), numParameters - options.K - 1), Dxx, zeros(size(Dxx, 1), 1)];
            b = [b; zeros(size(Dxx, 1), 1)];
        case 2    
            Dx = toeplitz([1 zeros(1, options.K - 2)], [1 -1 zeros(1, options.K - 2)]);
            A = [A; zeros(size(Dx, 1), numParameters - options.K), Dx, zeros(size(Dx, 1), 1)];
            b = [b; zeros(size(Dx, 1), 1)];
        case 3
            A = [A; zeros(options.K - 1, 4), eye(options.K - 1), zeros(options.K - 1, 1)];
            b = [b; zeros(options.K - 1, 1)];
    end;
end;

% positivity constraint on slack
A = [A; zeros(1, numParameters), 1];
b = [b; 0];
...
for t = 1:options.maxIters,
    % solve max-margin qp
    warning off;
    theta = quadprog(P, q, -1.0 * A, -1.0 * b, [], [], [], [], theta);
    warning on;
    ...
    % infer solution
    [y_hat, e_hat, z_hat] = linEnvInf(params.unaryWeight * instance.unary, ...
    ...
    [y_loss, e_loss, z_loss] = linEnvInf(params.unaryWeight * instance.unary - lossUnary, ...
        instance.pairwise, params.linEnvCoeffs, instance.cliques);
    % add constraint
    ...
    % update history
    history(t) = struct('params', params, 'y_hat', y_hat, 'z_hat', ...
        reshape(z_hat, options.K-1, instance.numCliques), ...
        'z_loss', reshape(z_loss, options.K-1, instance.numCliques));
    A = [A; phi - truePhi, 1];
    b = [b; loss];
end
